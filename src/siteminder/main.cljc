(ns siteminder.main
  "Kotodama WASM entrypoint for the Siteminder clean-room actor (L4) — Clojure port.
  ...
  facts (`siteminder.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "siteminder")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"siteminder.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "siteminder-compat" :tier tier :entities entities} 200])
...
(defn start! [] :siteminder-compat/ready)
